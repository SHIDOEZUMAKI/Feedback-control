...
def update_camera(pos, yaw):
    # Set camera position and target
    p.resetDebugVisualizerCamera(
        cameraDistance=camera_distance,  # Set to 0 as we're manually positioning the camera
        cameraYaw=yaw,
...
if __name__ == "__main__":
    ...
    yaw_rate_setpoint = 0
    # Add these variables to your main loop or inside the Simulator class
    # For camera controls
    camera_distance = 2.0  # Distance from drone
    camera_yaw = 0.0  # Horizontal angle around drone
    camera_pitch = -15.0  # Vertical angle (negative looks down)
    camera_follow = True  # Toggle for camera following
    ...
    while True:
        ...
        if ord('c') in keys and keys[ord('c')] & p.KEY_WAS_TRIGGERED:
            camera_follow = not camera_follow  # Toggle camera follow
        if camera_follow:
            update_camera(pos, yaw)
